...
const double rfitmax_o = 80.;
const double rfitmax_s = 60.;
const double rfitmax_l = 100.;
const double rfitmin = 2.;
...
double fitFunction(double *x, double *par)
{
  ...
  double Rcc_l = par[3];
  double N = par[4];
//  double Rcc_o = (Ro*pow(2.,1./alpha));
//  double Rcc_s = (Rs*pow(2.,1./alpha));
//  double Rcc_l = (Rl*pow(2.,1./alpha));
...
//        double distance = sqrt(ro*ro + rs*rs + rl*rl);
//        if (distance < rfitmin || distance > rfitmax) continue;
        if(ro < rfitmin) continue;
        if(rs < rfitmin) continue;
        if(rl < rfitmin) continue;
        double r_ellipsoid = sqrt(ro*ro/rfitmax_o/rfitmax_o+rs*rs/rfitmax_s/rfitmax_s+rl*rl/rfitmax_l/rfitmax_l);
        ...
        double expected = fitFunction(x, const_cast<double*>(params));
...
int main()
{
  ...
  TFile *file = TFile::Open("../EPOS_TH3_source_linbin.root");
  if (!file)
  {
  ...
  for (int ikt = 0; ikt < NKT; ++ikt)
  {
    ...
    TString histName = Form("pion_pair_source_avg_lcms_ikt%i", ikt);
    // Read the histogram
...
//    minimizer->SetFixedVariable(0, "alpha", 1.6);
    minimizer->SetVariable(0, "alpha", 1.3, 0.01);
    minimizer->SetVariable(1, "Rcc_out", 15.0, 0.01);
    minimizer->SetVariable(2, "Rcc_side", 15.0, 0.01);
    minimizer->SetVariable(3, "Rcc_long", 15.0, 0.01);
    minimizer->SetVariable(4, "N", 8., 0.01);
    ...
    std::cout << "conflev: " << TMath::Prob(chi2val, NDF) << endl;
